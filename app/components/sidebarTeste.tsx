    ...
    <div className="flex h-screen">
      ...
      >
        <nav className="flex flex-col gap-6 p-4">
          <MenuItem icon={<Home size={28} />} label="Início" open={open} active />
          <MenuItem icon={<Users size={28} />} label="Alunos" open={open} />
          <MenuItem icon={<Settings size={28} />} label="Configurações" open={open} />
        </nav>
      ...
      {/* CONTEÚDO */}
      <div
        className={`flex-1 flex flex-col px-4 py-6 transition-all duration-300
          ${open ? "sm:ml-[12%]" : "sm:ml-[4%]"} 
        `}
      >
...
function MenuItem({
  ...
  return (
    <div
      className={`flex items-center h-16 p-2 cursor-pointer transition-all duration-300 ${
        active ? "bg-brand-100 hover:bg-brand-200 rounded-lg" : "hover:bg-brand-200 rounded-lg"
      }`}
    >
      ...
      <div
        className={`overflow-hidden transition-all duration-300 ${
          open ? "w-auto ml-2" : "w-0"
        }`}
      >
        <span className="text-brand-950 text-2xl whitespace-nowrap">{label}</span>
      </div>
    </div>
  );
}
